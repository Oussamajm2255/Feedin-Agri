...
export class NotificationService {
  ...
  initSocket() {
    // Never initialize notifications for admin users
    if (this.auth.hasRole('admin')) {
      this.logger.debug('🚫 [WEBSOCKET] Skipping notification initialization for admin user');
      return;
    }
  ...
  notify(level: NotificationLevel, title: string, message?: string, options?: Partial<AppNotification>) {
    ...
    const alertOptions = { skipTranslation: options?.skipTranslation };
    
    if (level === 'critical') {
      this.alertService.error(title, alertMessage, undefined, alertOptions);
    } else if (level === 'info' || level === 'success') {
      this.alertService.success(title, alertMessage, undefined, alertOptions);
    } else if (level === 'warning') {
      this.alertService.warning(title, alertMessage, undefined, alertOptions);
    }
