...
export class AlertService {
   ...
   */
  success(title: string, message: string, duration: number = ALERT_DURATIONS.SUCCESS, options?: AlertOptions): Promise<CustomAlertResult> {
    const dismissible = options?.dismissible ?? true;
    const skipTranslation = options?.skipTranslation ?? false;
    this.show('success', title, message, duration, dismissible, skipTranslation);
   ...
   * Show an error alert
   */
  error(title: string, message: string, duration: number = ALERT_DURATIONS.ERROR, options?: AlertOptions): Promise<CustomAlertResult> {
    const dismissible = options?.dismissible ?? true;
    const skipTranslation = options?.skipTranslation ?? false;
    this.show('error', title, message, duration, dismissible, skipTranslation);
    return Promise.resolve({ isConfirmed: true, isDismissed: false });
  }
   ...
   */
  warning(title: string, message: string, duration: number = ALERT_DURATIONS.WARNING, options?: AlertOptions): Promise<CustomAlertResult> {
    const dismissible = options?.dismissible ?? true;
    const skipTranslation = options?.skipTranslation ?? false;
    this.show('warning', title, message, duration, dismissible, skipTranslation);
    return Promise.resolve({ isConfirmed: true, isDismissed: false });
  }
   ...
   * Show an info alert
   */
  info(title: string, message: string, duration: number = ALERT_DURATIONS.INFO, options?: AlertOptions): Promise<CustomAlertResult> {
    const dismissible = options?.dismissible ?? true;
    const skipTranslation = options?.skipTranslation ?? false;
    this.show('info', title, message, duration, dismissible, skipTranslation);
    return Promise.resolve({ isConfirmed: true, isDismissed: false });
  }
