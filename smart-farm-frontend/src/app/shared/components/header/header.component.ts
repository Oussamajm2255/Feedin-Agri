...
export class HeaderComponent implements OnInit, OnDestroy {
  ...
  constructor() {
    ...
    // Only initialize notifications for farmers and moderators
    if (this.authService.hasAnyRole(['farmer', 'moderator'])) {
      ...
      // Admin and other users don't see notifications
      this.unreadCount = 0;
    }
  ...
  // Screen size detection is now handled by BreakpointService
  // No need for manual resize listeners or checkScreenSize()
  ...
  getUnreadBadgeDisplay(): string {
    const count = this.getUnreadBadge();
  ...
  private async refreshUnread(): Promise<void> {
    // Only refresh notifications for farmers and moderators
    if (!this.authService.hasAnyRole(['farmer', 'moderator'])) {
      return;
    }
  ...
  getCurrentLanguageDisplay(): string {
    ...
    const language = this.languageService.getAvailableLanguages().find(lang => lang.code === currentCode);
    return language ? language.nativeName : currentCode.toUpperCase();
  }
