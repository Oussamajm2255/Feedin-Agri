    ...
    if (ownerId) {
      ...
      if (devices.length === 0 && totalCount > 0) {
        ...
        if (devicesNoRelations.length > 0) {
          // Batch-load all farms in ONE query instead of N+1
          const farmIds = [...new Set(devicesNoRelations.map(d => d.farm_id).filter(Boolean))];
          const farms = farmIds.length > 0
            ? await this.farmRepository.find({ where: { farm_id: In(farmIds) } })
            : [];
          const farmMap = new Map(farms.map(f => [f.farm_id, f]));

          for (const device of devicesNoRelations) {
            (device as any).farm = farmMap.get(device.farm_id) || null;
          }
