...
export class AdminService {
  ...
  /**
   * Get all pending access requests (users with status = 'pending')
   */
  async getPendingAccessRequests(page: number = 1, limit: number = 20) {
    const skip = (page - 1) * limit;
   ...
   * Activates the user account and emits real-time events
   */
  async approveAccessRequest(userId: string, adminUserId: string) {
    ...
    this.eventEmitter.emit('access.approved', {
      userId: user.user_id,
      email: user.email,
    ...
    // Note: AdminNotificationsService listens for 'access.approved' and
    // handles persisting the audit trail + auto-resolving the registration notification.
   ...
   * Suspends the user account and emits real-time events
   */
  async rejectAccessRequest(userId: string, adminUserId: string, reason: string) {
    const user = await this.userRepository.findOne({
      where: { user_id: userId },
    });
    ...
    // Emit events for real-time notifications
    this.eventEmitter.emit('access.rejected', {
      userId: user.user_id,
      email: user.email,
      firstName: user.first_name,
      lastName: user.last_name,
    ...
    // Note: AdminNotificationsService listens for 'access.rejected' and
    // handles persisting the audit trail + auto-resolving the registration notification.
