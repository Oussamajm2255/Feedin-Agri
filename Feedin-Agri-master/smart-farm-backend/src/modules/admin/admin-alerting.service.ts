...
export class AdminAlertingService implements OnModuleInit, OnModuleDestroy {
  ...
  // Note: user.registered is handled by AdminNotificationsService
  // with better context (suggestedActions, user_registration type for approve/reject flow).
  // Removed duplicate handler here to avoid creating 2 notifications per registration.
  ...
  async handleActionExecutionFailed(data: { actionId?: string; deviceId?: string; device_id?: string; error?: string; farmId?: string; farm_id?: string; message?: string }) {
    const deviceId = data.deviceId || data.device_id || 'unknown';
    const alertKey = `action_failed_${deviceId}`;
    if (!this.shouldCreateAlert(alertKey)) return;
    ...
    await this.adminNotifications.create({
      ...
      message: `Failed to execute action on device ${deviceId}: ${errorMsg}`,
      context: {
        actionId: data.actionId,
        deviceId: deviceId,
        error: errorMsg,
        farmId: data.farmId || data.farm_id,
        suggestedActions: [
