...
export class AllExceptionsFilter implements ExceptionFilter {
  private readonly logger = new Logger(AllExceptionsFilter.name);
    ...
    if (status === 401) {
      // 401 on /auth/me is a routine session check — use debug level
      // 401 on other endpoints is worth a warning
      const isSessionCheck = request.url.includes('/auth/me');
      const logMethod = isSessionCheck ? 'debug' : 'warn';
      this.logger[logMethod](
        `${request.method} ${request.url} - ${exception instanceof Error ? exception.message : 'Unauthorized'}`,
      );
