...
export class ActionEventsService {
  ...
  start() {
    ...
    interval(15000).pipe(startWith(0)).subscribe(async () => {
      // Skip polling when not authenticated to avoid 401 floods
        ...
        for (const a of items) {
          ...
          if (a.status === 'error') {
            if (this.notifications.shouldNotify(key, 'critical')) {
              this.notifications.notify('critical', 'actionStream.actionFailed', `${a.device_id} • ${a.action_uri}`, { source: 'action', context: a, skipTranslation: true });
            }
          } else if (a.status === 'sent' || a.status === 'ack') {
            if (this.notifications.shouldNotify(key, 'success')) {
              this.notifications.notify('success', 'actionStream.actionExecuted', `${a.device_id} • ${a.action_uri}`, { source: 'action', context: a, skipTranslation: true });
            }
          }
        }
