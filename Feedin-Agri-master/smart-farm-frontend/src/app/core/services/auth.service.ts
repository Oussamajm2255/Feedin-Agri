...
export class AuthService {
  ...
  logout(navigate: boolean = true): void {
    // ✅ Flush the HTTP response cache FIRST — prevents stale authenticated data
    // (sensor readings, farm stats, user profile) from leaking into the next
    // user session within the same browser tab.
    clearHttpCache();

    // Try to clear the server cookie; tolerate failures (e.g., already-expired session)
