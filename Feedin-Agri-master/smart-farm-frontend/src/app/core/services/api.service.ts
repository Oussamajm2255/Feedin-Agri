...
export class ApiService {
   ...
   * 
   * Expected query params: limit?: number, offset?: number
   * Expected response format: ListActionsResponse
   * 
   * Note: If backend doesn't support this endpoint yet (404/501), 
   * it will return empty result gracefully.
   * Frontend handles this by showing "No activity" message.
   */
  getActionsByUser(userId: string, limit?: number, offset?: number): Observable<ListActionsResponse> {
    let params = new HttpParams();
    if (limit) params = params.set('limit', limit.toString());
    if (offset) params = params.set('offset', offset.toString());
    
    // Try the user-specific endpoint first
    return this.http.get<ListActionsResponse>(`${this.API_URL}/actions/by-user/${userId}`, { params }).pipe(
      catchError((error) => {
        // If endpoint doesn't exist (404) or not implemented (501), return empty result
        if (error.status === 404 || error.status === 501) {
          console.warn(`Endpoint /actions/by-user/${userId} not yet implemented. Returning empty result.`);
  ...
  /**
   * Get action status by ID - used for polling ACK status
   * Returns the action with its current status (sent, ack, error, timeout)
   */
  getActionStatus(id: string): Observable<{ status: string; action?: ActionLog }> {
    return this.http.get<ActionLog>(`${this.API_URL}/actions/${id}`).pipe(
      map(action => ({ status: action?.status || 'unknown', action })),
      catchError(error => {
        console.warn('Error fetching action status:', error);
        return of({ status: 'unknown' });
      })
    );
  }

  executeAction(body: ExecuteActionRequest): Observable<{ ok: boolean; action_log_id?: string; id?: string }> {
    return this.http.post<{ ok: boolean; action_log_id?: string; id?: string }>(`${this.API_URL}/actions/execute`, body);
  }
  ...
  requestFarmAccess(): Observable<{ message: string }> {
    return this.http.post<{ message: string }>(`${this.API_URL}/farms/request-access`, {});
  }
  ...
  /**
   * Get optimized crop dashboard data in a single call
   * Returns crop, sensors with readings, pre-calculated KPIs, health status, statistics
   */
  getCropDashboard(id: string): Observable<any> {
    return this.http.get<any>(`${this.API_URL}/crops/${id}/dashboard`);
  }

  /**
   * Get sustainability metrics for a crop
   * Returns water saved, energy saved, carbon offset, efficiency scores
   */
  getCropSustainability(id: string): Observable<any> {
    return this.http.get<any>(`${this.API_URL}/crops/${id}/sustainability`);
  }

  /**
   * Get comparison metrics for a crop
   * @param mode - 'farm_avg' | 'last_season' | 'other_crop'
   * @param compareCropId - Required when mode is 'other_crop'
   */
  getCropComparison(
    id: string,
    ...
    if (compareCropId) {
      params = params.set('compareCropId', compareCropId);
    }
    return this.http.get<any>(`${this.API_URL}/crops/${id}/comparison`, { params });
  }
  ...
  getZones(farmId: string): Observable<any[]> {
    ...
    return this.http.get<any[]>(`${this.API_URL}/zones`, { params });
  }

  getZone(id: string): Observable<any> {
    return this.http.get<any>(`${this.API_URL}/zones/${id}`);
  }

  createZone(data: any): Observable<any> {
    return this.http.post<any>(`${this.API_URL}/zones`, data);
  }

  updateZone(id: string, data: any): Observable<any> {
    return this.http.patch<any>(`${this.API_URL}/zones/${id}`, data);
  }

  deleteZone(id: string): Observable<{ message: string }> {
    return this.http.delete<{ message: string }>(`${this.API_URL}/zones/${id}`);
  }

  // ── Dashboard aggregation ──────────────────────────────────

  getFarmDashboard(farmId: string): Observable<any> {
    return this.http.get<any>(`${this.API_URL}/dashboard/farm/${farmId}`);
  }

  getZoneDashboard(zoneId: string): Observable<any> {
    return this.http.get<any>(`${this.API_URL}/dashboard/zone/${zoneId}`);
  }

  // ── Digital Twin endpoints ──────────────────────────────────

  getDigitalTwins(farmId: string): Observable<any[]> {
    const params = new HttpParams().set('farmId', farmId);
    return this.http.get<any[]>(`${this.API_URL}/digital-twins`, { params });
  }

  getDigitalTwin(id: string): Observable<any> {
    return this.http.get<any>(`${this.API_URL}/digital-twins/${id}`);
  }

  createDigitalTwin(data: any): Observable<any> {
    return this.http.post<any>(`${this.API_URL}/digital-twins`, data);
  }

  updateDigitalTwin(id: string, data: any): Observable<any> {
    return this.http.patch<any>(`${this.API_URL}/digital-twins/${id}`, data);
  }

  deleteDigitalTwin(id: string): Observable<{ message: string }> {
    return this.http.delete<{ message: string }>(`${this.API_URL}/digital-twins/${id}`);
  }

  uploadDigitalTwinMedia(id: string, file: File): Observable<any> {
    const formData = new FormData();
    formData.append('file', file);
    return this.http.post<any>(`${this.API_URL}/digital-twins/${id}/media`, formData);
  }
}
