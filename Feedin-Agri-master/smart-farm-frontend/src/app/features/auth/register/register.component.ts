import { Component, inject, ViewChild, ElementRef, signal, AfterViewInit, OnDestroy } from '@angular/core';
...
export class RegisterComponent implements AfterViewInit, OnDestroy {
  ...
  ngOnDestroy(): void {
    ...
    if (video) {
      video.removeEventListener('loadeddata', this.boundOnVideoLoaded);
      video.removeEventListener('timeupdate', this.boundOnTimeUpdate);
      video.removeEventListener('error', this.boundOnVideoError);
    }
  }

  // ── Video playback (play once → freeze at duration − 1s) ──
  private boundOnVideoLoaded = this.onVideoLoaded.bind(this);
  ...
  private setupVideoPlayback(): void {
    ...
    const video = this.bgVideo?.nativeElement;
    if (!video) {
      console.warn('Register: video element not found');
      this.videoError.set(true);
      return;
    }

    video.addEventListener('loadeddata', this.boundOnVideoLoaded);
    video.addEventListener('timeupdate', this.boundOnTimeUpdate);
    video.addEventListener('error', this.boundOnVideoError);

    video.muted = true;
    video.play()?.catch(() => {
      console.warn('Register: video autoplay prevented');
      video.currentTime = 0;
    });
  }
  ...
  private onVideoError(): void {
    this.videoError.set(true);
    console.warn('Register: video failed to load');
  }
