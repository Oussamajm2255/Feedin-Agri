...
export class AdminSettingsComponent implements OnInit {
  ...
  private showToast(message: string, type: 'success' | 'error' | 'info' | 'warning') {
    ...
    if (type === 'success') {
      this.alertService.success(titleMap[type], message, 3000);
    } else if (type === 'error') {
      this.alertService.error(titleMap[type], message, 3000);
    } else if (type === 'warning') {
      this.alertService.warning(titleMap[type], message, 3000);
    } else {
      this.alertService.info(titleMap[type], message, 3000);
    }
  }
