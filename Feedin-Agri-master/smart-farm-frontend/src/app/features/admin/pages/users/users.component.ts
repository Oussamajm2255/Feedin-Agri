...
export class UsersComponent implements OnInit, AfterViewInit {
  ...
  showSnackbar(message: string, type: 'success' | 'error' | 'warn' | 'info' = 'info'): void {
    ...
    if (type === 'success') {
      ...
      this.alertService.error(titleMap[type], message, 4000);
    } else if (type === 'warn') {
      this.alertService.warning(titleMap[type], message, 4000);
    } else {
      this.alertService.info(titleMap[type], message, 4000);
    }
  }
