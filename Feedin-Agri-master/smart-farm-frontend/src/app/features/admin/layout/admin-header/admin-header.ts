...
import { AdminLayoutService } from '../../../../admin/core/services/admin-layout.service';
...
export class AdminHeader implements OnInit, OnDestroy {
  ...
  public adminWs = inject(AdminWebSocketService);
  ...
  ngOnInit(): void {
    ...
    // Load initial counts from API (only if authenticated)
    ...
    this.adminWs.countsUpdated$
      ...
      .subscribe((data: any) => {
        if (data.action === 'approved' || data.action === 'rejected') {
          this.pendingAccessCount = Math.max(0, this.pendingAccessCount - 1);
        }
      });

    // Keep HTTP polling as a heartbeat sync (every 60s), only when authenticated
    interval(60000)
      .pipe(takeUntil(this.destroy$))
      .subscribe(() => {
        if (this.authService.isAuthenticated()) {
  ...
  private loadNotificationCount(): void {
    this.adminApi.get<any>('admin/notifications/counts')
      ...
      .subscribe(counts => {
        ...
        // Sync with WebSocket service
        this.adminWs.setNotificationCount(this.notificationCount);
      });
  }

  private loadPendingAccessCount(): void {
    this.adminApi.getPendingAccessRequests(1, 1)
      .pipe(
        takeUntil(this.destroy$),
        catchError(() => of({ total: 0 }))
      )
      .subscribe((result: any) => {
        this.pendingAccessCount = result.total || 0;
        this.adminWs.setPendingAccessCount(this.pendingAccessCount);
      });
  }
