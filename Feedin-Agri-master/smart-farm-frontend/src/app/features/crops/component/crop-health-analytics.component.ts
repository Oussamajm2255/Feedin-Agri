...
import { CropService, SensorWithReading, KPIFilter } from '../../../core/services/crop.service';
  ...
  template: `
    <mat-card class="health-analytics">
      ...
      <mat-card-content>
        ...
        <!-- Empty State -->
        <div *ngIf="!loading() && !hasVisibleSensors()" class="empty-state">
          <mat-icon>sensors_off</mat-icon>
          <h3>{{ 'crops.health.empty.title' | translate }}</h3>
          <p>{{ activeFilter() !== 'all' ? ('crops.health.empty.filtered' | translate) : ('crops.health.empty.description' | translate) }}</p>
        </div>

        <!-- Charts -->
        <div *ngIf="!loading() && hasVisibleSensors()" class="charts-container">
          <mat-tab-group>
            <!-- Moisture Chart -->
            <mat-tab *ngIf="moistureSensors().length > 0">
              ...
              <div class="chart-wrapper">
                <div class="chart-container" *ngIf="!chartLoading(); else chartLoadingTpl">
                  <div echarts [options]="moistureChartOptions()" class="chart"></div>
                </div>
            ...
            <mat-tab *ngIf="temperatureSensors().length > 0">
              ...
              <div class="chart-wrapper">
                <div class="chart-container" *ngIf="!chartLoading(); else chartLoadingTpl">
                  <div echarts [options]="temperatureChartOptions()" class="chart"></div>
                </div>
                <div class="sensor-legend">
            ...
            <ng-template #chartLoadingTpl>
              <div class="chart-loading">
                ...
                <span>{{ 'crops.health.loadingChart' | translate }}</span>
              </div>
            </ng-template>
          </mat-tab-group>
        </div>
...
export class CropHealthAnalyticsComponent implements OnInit {
  ...
  activeFilter = input<KPIFilter>('all'); // KPI filter from parent
  ...
  // Computed sensors by type - respects KPI filter
  moistureSensors = computed(() => {
    const filter = this.activeFilter();
    if (filter !== 'all' && filter !== 'moisture') return [];
    return this.sensors().filter(s => this.isSensorType(s, 'moisture'));
  });

  temperatureSensors = computed(() => {
    const filter = this.activeFilter();
    if (filter !== 'all' && filter !== 'temperature') return [];
    return this.sensors().filter(s => this.isSensorType(s, 'temp'));
  });

  humiditySensors = computed(() => {
    const filter = this.activeFilter();
    if (filter !== 'all' && filter !== 'humidity') return [];
    return this.sensors().filter(s => this.isSensorType(s, 'humidity') || this.isSensorType(s, 'humid'));
  });

  // Check if any sensors are visible based on filter
  hasVisibleSensors = computed(() =>
    this.moistureSensors().length > 0 ||
  ...
  private loadSensorData(forceRefresh = false): void {
    ...
    forkJoin(readingRequests)
      ...
      .subscribe({
        next: (results: Array<{ sensorId: string; readings: SensorReading[] }>) => {
          const readingsMap = new Map<string, SensorReading[]>();
          results.forEach((result) => {
            readingsMap.set(result.sensorId, result.readings);
          });
  ...
  private buildChartOptions(
    ...
    const series: any[] = sensors.map((sensor) => {
