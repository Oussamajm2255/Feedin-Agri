...
import { KPIFilter } from '../../../core/services/crop.service';
...
export class CropEventsTimelineComponent implements OnInit {
  ...
  kpiFilter = input<KPIFilter>('all'); // KPI filter from parent dashboard
  ...
  filteredGroups = computed(() => {
    ...
    const kpiFilter = this.kpiFilter();
    ...
    return groups
      .map(group => ({
        ...
        events: group.events.filter(e => {
          // Apply type filter
          if (filter !== 'all') {
            if (filter === 'actions' && e.type !== 'action') return false;
            if (filter === 'alerts' && e.type !== 'alert') return false;
            if (filter === 'notifications' && e.type !== 'notification') return false;
          }

          // Apply KPI filter - filter events by related sensor/metric type
          if (kpiFilter !== 'all') {
            const eventText = `${e.title} ${e.description} ${e.source}`.toLowerCase();
            if (kpiFilter === 'moisture' && !eventText.includes('moisture') && !eventText.includes('water') && !eventText.includes('irrigation')) return false;
            if (kpiFilter === 'temperature' && !eventText.includes('temp') && !eventText.includes('heat')) return false;
            if (kpiFilter === 'humidity' && !eventText.includes('humid')) return false;
            if (kpiFilter === 'healthy' && !eventText.includes('health') && !eventText.includes('status')) return false;
            if (kpiFilter === 'stressed' && !eventText.includes('stress') && !eventText.includes('warning') && !eventText.includes('alert')) return false;
          }
