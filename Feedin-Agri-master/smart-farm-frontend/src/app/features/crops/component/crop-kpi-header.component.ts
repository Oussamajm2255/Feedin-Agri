...
/**
 * Threshold configuration for KPI warnings and critical alerts
 * Per-crop customizable thresholds
 */
export interface KPIThresholds {
  moisture: {
    warningLow: number;   // Below this = warning (default: 30)
    warningHigh: number;  // Above this = warning (default: 70)
    criticalLow: number;  // Below this = critical (default: 20)
    criticalHigh: number; // Above this = critical (default: 90)
  };
  temperature: {
    warningLow: number;   // Below this = warning (default: 10)
    warningHigh: number;  // Above this = warning (default: 35)
    criticalLow: number;  // Below this = critical (default: 5)
    criticalHigh: number; // Above this = critical (default: 40)
  };
  humidity: {
    warningLow: number;   // Below this = warning (default: 40)
    warningHigh: number;  // Above this = warning (default: 80)
    criticalLow: number;  // Below this = critical (default: 30)
    criticalHigh: number; // Above this = critical (default: 90)
  };
}

/**
 * Default thresholds for common crops
 * Can be overridden per-crop
 */
  ...
  template: `
    <section class="kpi-header">
      ...
      <mat-card
        ...
        [matTooltip]="('crops.kpi.tooltips.soilMoisture' | translate) + '\n' + getThresholdInfo('moisture')">
        <mat-card-content>
          <div class="kpi-icon" [class.warning]="isWarning('moisture')" [class.critical]="isCritical('moisture')">
          ...
          <div class="kpi-data">
            ...
            <span class="kpi-trend" *ngIf="kpis().avgSoilMoisture !== null">
              <mat-icon [class.warning]="isWarning('moisture')" [class.critical]="isCritical('moisture')">
                {{ isCritical('moisture') ? 'error' : (isWarning('moisture') ? 'warning' : (getMoistureStatus() === 'low' ? 'trending_down' : 'trending_up')) }}
              </mat-icon>
            </span>
          </div>
          <div class="ripple"></div>
      ...
      <mat-card
        ...
        [class.critical]="isCritical('temperature')"
        [class.warning]="isWarning('temperature')"
        (click)="onCardClick('temperature')"
        [matTooltip]="('crops.kpi.tooltips.temperature' | translate) + '\n' + getThresholdInfo('temperature')">
        <mat-card-content>
          <div class="kpi-icon warm" [class.warning]="isWarning('temperature')" [class.critical]="isCritical('temperature')">
          ...
          <div class="kpi-data">
            ...
            <span class="kpi-trend" *ngIf="kpis().avgTemperature !== null && (isWarning('temperature') || isCritical('temperature'))">
              <mat-icon [class.warning]="isWarning('temperature')" [class.critical]="isCritical('temperature')">
                {{ isCritical('temperature') ? 'error' : 'warning' }}
              </mat-icon>
            </span>
          </div>
          <div class="ripple"></div>
      ...
      <mat-card
        ...
        [class.critical]="isCritical('humidity')"
        [class.warning]="isWarning('humidity')"
        (click)="onCardClick('humidity')"
        [matTooltip]="('crops.kpi.tooltips.humidity' | translate) + '\n' + getThresholdInfo('humidity')">
        <mat-card-content>
          <div class="kpi-icon" [class.warning]="isWarning('humidity')" [class.critical]="isCritical('humidity')">
          ...
          <div class="kpi-data">
            ...
            <span class="kpi-trend" *ngIf="kpis().avgHumidity !== null && (isWarning('humidity') || isCritical('humidity'))">
              <mat-icon [class.warning]="isWarning('humidity')" [class.critical]="isCritical('humidity')">
                {{ isCritical('humidity') ? 'error' : 'warning' }}
              </mat-icon>
            </span>
          </div>
          <div class="ripple"></div>
...
export class CropKpiHeaderComponent {
  ...
  thresholds = input<KPIThresholds>(DEFAULT_KPI_THRESHOLDS); // Configurable thresholds

  // Outputs
  filterChange = output<KPIFilter>();

  // Computed thresholds with defaults fallback
  effectiveThresholds = computed(() => {
    const custom = this.thresholds();
    return {
      moisture: { ...DEFAULT_KPI_THRESHOLDS.moisture, ...custom?.moisture },
      temperature: { ...DEFAULT_KPI_THRESHOLDS.temperature, ...custom?.temperature },
      humidity: { ...DEFAULT_KPI_THRESHOLDS.humidity, ...custom?.humidity }
    };
  });
  ...
  /**
   * Check if a metric is in critical range
   * Uses configurable thresholds
   */
  isCritical(type: 'moisture' | 'temperature' | 'humidity'): boolean {
    const kpis = this.kpis();
    const thresholds = this.effectiveThresholds();
    
    if (type === 'moisture') {
      const t = thresholds.moisture;
      return kpis.avgSoilMoisture !== null && 
             (kpis.avgSoilMoisture < t.criticalLow || kpis.avgSoilMoisture > t.criticalHigh);
    }
    if (type === 'temperature') {
      const t = thresholds.temperature;
      return kpis.avgTemperature !== null && 
             (kpis.avgTemperature < t.criticalLow || kpis.avgTemperature > t.criticalHigh);
    }
    if (type === 'humidity') {
      const t = thresholds.humidity;
      return kpis.avgHumidity !== null && 
             (kpis.avgHumidity < t.criticalLow || kpis.avgHumidity > t.criticalHigh);
    }
    return false;
  }

  /**
   * Check if a metric is in warning range
   * Uses configurable thresholds - warning is the zone between normal and critical
   */
  isWarning(type: 'moisture' | 'temperature' | 'humidity'): boolean {
    const kpis = this.kpis();
    const thresholds = this.effectiveThresholds();
    
    // If already critical, not a warning
    if (this.isCritical(type)) return false;
    
    if (type === 'moisture') {
      const t = thresholds.moisture;
      return kpis.avgSoilMoisture !== null &&
             ((kpis.avgSoilMoisture >= t.criticalLow && kpis.avgSoilMoisture < t.warningLow) ||
              (kpis.avgSoilMoisture > t.warningHigh && kpis.avgSoilMoisture <= t.criticalHigh));
    }
    if (type === 'temperature') {
      const t = thresholds.temperature;
      return kpis.avgTemperature !== null &&
             ((kpis.avgTemperature >= t.criticalLow && kpis.avgTemperature < t.warningLow) ||
              (kpis.avgTemperature > t.warningHigh && kpis.avgTemperature <= t.criticalHigh));
    }
    if (type === 'humidity') {
      const t = thresholds.humidity;
      return kpis.avgHumidity !== null &&
             ((kpis.avgHumidity >= t.criticalLow && kpis.avgHumidity < t.warningLow) ||
              (kpis.avgHumidity > t.warningHigh && kpis.avgHumidity <= t.criticalHigh));
    }
    return false;
  }

  /**
   * Get threshold info for tooltip display
   */
  getThresholdInfo(type: 'moisture' | 'temperature' | 'humidity'): string {
    const thresholds = this.effectiveThresholds();
    const t = thresholds[type];
    const unit = type === 'temperature' ? '°C' : '%';
    
    return this.languageService.translate('crops.kpi.thresholdInfo', {
