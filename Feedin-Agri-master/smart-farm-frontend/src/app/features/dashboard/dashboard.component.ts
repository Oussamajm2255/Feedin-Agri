...
import { ZoneCardComponent } from '../../shared/components/zone-card/zone-card.component';
import { ZoneAlertsComponent } from '../../shared/components/zone-alerts/zone-alerts.component';
...
import { Zone, FarmDashboardAggregation, ZoneDashboardData } from '../../core/models/zone.model';
...
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  public zonesService = inject(ZonesService);
  ...
  // Zone-centric dashboard data
  zoneDashboardData: FarmDashboardAggregation | null = null;
  ...
  loadFarmData(farm: Farm): void {
    ...
    // Load zone-centric dashboard data
    this.loadZoneDashboard(farm.farm_id);
  }
  ...
  // ── Zone-centric methods ─────────────────────────────────

  private loadZoneDashboard(farmId: string): void {
  ...
  onZoneSelected(zone: Zone | null): void {
    // Zone filtering is handled reactively via getFilteredZones()
    // No additional API call needed — data is already loaded
  }

  getFilteredZones(): ZoneDashboardData[] {
    if (!this.zoneDashboardData) return [];

    const selected = this.zonesService.selectedZone();
    if (!selected) {
      return this.zoneDashboardData.zones;
    }
    return this.zoneDashboardData.zones.filter((z) => z.zone.id === selected.id);
  }
