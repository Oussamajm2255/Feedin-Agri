...
import { DigitalTwinManageDialogComponent } from './digital-twin-manage.dialog';
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, MatTooltipModule, MatIconModule, MatDialogModule, MatButtonModule, MatSelectModule, FormsModule],
...
export class DigitalTwinComponent implements OnInit, OnDestroy {
  ...
  // ============================================================================
  readonly twins = signal<any[]>([]);
  readonly activeTwinId = signal<string | null>(null);
  
  readonly activeTwin = computed(() => {
    const list = this.twins();
    const active = this.activeTwinId();
    if (!active) return list.length ? list[0] : null;
    return list.find(t => t.id === active) || (list.length ? list[0] : null);
  });
  ...
  readonly hasPositions = computed(() => {
    ...
    if (twin && twin.config && Object.keys(twin.config).length > 0) return true;
    return this.actuatorPositions() !== null;
  });
  ...
  readonly greenhouseImage = computed(() => {
    const twin = this.activeTwin();
    // If backend URL exists (might be a relative URL, prepend server domain if needed based on setup)
    if (twin && twin.media_url) {
      if (twin.media_url.startsWith('http')) return twin.media_url;
      // Fetching from localhost or currently deployed URL based on environment.apiUrl
      // Since API_URL might be different, if it's relative like /api/v1/digital-twins/media/..., we must use the configured baseUrl
      // Because we can't inject environment directly inside computed without importing it, we assume proxy or direct backend URL
      // Since it's a relative path starting with /api, we can just use it directly! (Angular proxy or same domain)
      return twin.media_url;
    }
  ...
  private loadTwinsFromDb(): void {
    const selectedFarm = this.farmManagement.getSelectedFarm();
    if (!selectedFarm) return;
    
    this.apiService.getDigitalTwins(selectedFarm.farm_id).subscribe({
      next: (res) => {
        this.twins.set(res);
      },
      error: (err) => {
        this.error('Failed to load dynamic twins', err);
      }
    });
  }

  // Opens the Twin Manager dialog
  openManageDialog(): void {
    const selectedFarm = this.farmManagement.getSelectedFarm();
    if (!selectedFarm) return;

    const dialogRef = this.dialog.open(DigitalTwinManageDialogComponent, {
      width: '800px',
  ...
  // Change the active twin via dropdown
  onSwitchTwin(twinId: string): void {
    this.activeTwinId.set(twinId);
  }
  ...
  // ============================================================================
   ...
   getActuatorStyle(type: string | ActuatorType): { [key: string]: string } {
     ...
     const twin = this.activeTwin();
     let positions = this.actuatorPositions();

     // Use dynamic config if present
     if (twin && twin.config && Object.keys(twin.config).length > 0) {
        positions = twin.config as any;
     }
