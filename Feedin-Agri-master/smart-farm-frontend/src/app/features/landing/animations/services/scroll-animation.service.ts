...
import { Injectable, signal, OnDestroy, NgZone, afterNextRender } from '@angular/core';
...
export class ScrollAnimationService implements OnDestroy {
  ...
  private velocityHandler: (() => void) | null = null;
  ...
  createReveal(config: ScrollRevealConfig): ScrollTrigger | null {
    ...
    if (animation === 'stagger' && element.children.length > 0) {
      // For stagger, set initial state on children, NOT the container
      ...
      // For normal animations, set initial state on the element itself
  ...
  ngOnDestroy(): void {
    ...
    // Remove velocity tracking listener
    if (this.velocityHandler) {
      window.removeEventListener('scroll', this.velocityHandler);
  ...
  private setupVelocityTracking(): void {
    ...
    this.velocityHandler = trackVelocity;
