...
export class ActionsService {
   ...
   * Joins action_logs -> devices -> farms -> filters by owner_id
   */
  async getByUser(userId: string, limit?: number, offset?: number) {
    const qb = this.repo
      .createQueryBuilder('action')
      .leftJoin('devices', 'device', 'device.device_id = action.device_id')
      .leftJoin('farms', 'farm', 'farm.farm_id = device.farm_id')
      .where('farm.owner_id = :userId', { userId })
